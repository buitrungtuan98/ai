...
| `database/types.py` | data | `EncryptedString` TypeDecorator + enums (incl. AWAITING_REVIEW / awaiting_review / rejected) | column values | encrypted columns | security | DONE | 2026-07-18 |
| `database/models.py` | data | ORM: Users→Channels→Campaigns→{Tasks,BufferPool}; task timing/retry/published-url columns | — | ORM classes | SQLAlchemy, types | DONE | 2026-07-18 |
...
| `core/captions.py` | render | Word-by-word AND line-style ASS subtitles; PIL 1080px wrapping (shared font loader) | timings, text, style | `.ass` file | Pillow | DONE | 2026-07-18 |
...
| `core/video_factory.py` | render | Orchestration: TTS→footage→captions→render→stitch(copy, music mix)→thumbnail→branding→cleanup; honored A/B toggle | script, episode, cfg | master.mp4 + thumb + metadata | all core/* , config | DONE | 2026-07-18 |
...
| `workers/video_worker.py` | worker | render_task (buffer + auto-publish or review) + publish_task (approve/retry); hydration (per-campaign size), state machine, timing/published-url, error→Telegram | Task/Buffer id | published video, DB updates | video_factory, services, models | DONE | 2026-07-18 |
...
| `workers/scheduler.py` | worker | Daemon tick: TIMEZONE-aware slot-gated hydration, buffer expiry, disk sweep | active campaigns | enqueued jobs, cleanup | video_worker, cleanup, config | DONE | 2026-07-18 |
...
| `main.py` | web | FastAPI app: login/session, dashboards (health strip, counts), campaigns CRUD+edit, asset preview/approve/reject + ranged streaming, task API + retry, credential tests, OAuth flows | HTTP | HTML/JSON | fastapi, auth, models, task_queue | DONE | 2026-07-18 |
