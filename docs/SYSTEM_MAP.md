...
| `workers/scheduler.py` | worker | Daemon tick: stuck-task reaper, eager hydration, slot-timed publishing, buffer expiry, disk sweep, daily learning pass + min-per-day watchdog (Telegram alert when a campaign publishes below its minimum) | active campaigns | jobs, cleanup, playbooks, alerts | video_worker, analytics_service, ai_engine | DONE | 2026-07-20 |
