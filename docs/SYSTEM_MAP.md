...
| `workers/video_worker.py` | worker | render_task (buffer + continuous/slot-scheduled/review) + publish_task; Auto-QC gate (vet footage, judge master, re-render once, else park for review); hydration with per-campaign daily render cap (`max_per_day`, quota rationing); persona+memory pass-through, synopsis store; state machine, error→Telegram | Task/Buffer id | published video, DB updates | video_factory, qc, services, models | DONE | 2026-07-20 |
...
| `workers/scheduler.py` | worker | Daemon tick: stuck-task reaper, eager hydration, slot-timed publishing, buffer expiry, disk sweep, daily learning pass + min-per-day watchdog + operator heartbeat digest (24h summary, quota, disk via Telegram) | active campaigns | jobs, cleanup, playbooks, alerts | video_worker, analytics_service, ai_engine, usage | DONE | 2026-07-20 |
