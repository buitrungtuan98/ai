...
| `core/ai_engine.py` | ai | `generate_structured` Gemini wrapper; persona/style/catchphrase system-prompt composer; episode-memory prompts (no_repeat/serial); `VideoScript`(+synopsis)/`MetadataSet` | topic, campaign cfg, prior synopses | script + 3 A/B metadata | google-generativeai, pydantic | DONE | 2026-07-18 |
...
| `workers/scheduler.py` | worker | Daemon tick: eager hydration, slot-timed one-per-slot publishing (per-campaign tz, double-post guard), buffer expiry, disk sweep | active campaigns | enqueued jobs, cleanup | video_worker, task_queue, config | DONE | 2026-07-18 |
