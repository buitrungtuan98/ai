...
| `core/video_factory.py` | render | Orchestration: TTS→footage (EN keywords + fallback chain)→motion→captions→render→stitch(copy, music)→thumbnail→cleanup | script, episode, cfg | master.mp4 + thumb + metadata | all core/*, config | DONE | 2026-07-19 |
