...
| `database/models.py` | data | ORM: Users→Channels→Campaigns→{Tasks,BufferPool}; user `gemini_model` chain; task timing/retry/published-url/synopsis/ab_variant columns | — | ORM classes | SQLAlchemy, types | DONE | 2026-07-22 |
...
| `core/ai_engine.py` | ai | Gemini wrapper (fallback chain, quota fail-fast, call metering); persona+playbook prompt composer; episode memory (synopsis REQUIRED in schema); critic loop (incl. grammar/diacritics gate); playbook distiller; vision judges (single + batched footage, final QC — audio-aware when a track is attached); campaign designer (voices derived from `tts.VOICE_CHOICES`); duration targeting (word budget + length fix); live model listing + curated free-tier limit catalog (`GEMINI_MODEL_CATALOG`); schemas | topic, cfg, learning, frames, audio | script + metadata + critiques + playbooks + verdicts + proposals + model lists | google-generativeai, pydantic, Pillow, config, usage, tts | DONE | 2026-07-22 |
...
| `main.py` | web | FastAPI app: login/session, dashboard (health/counts + AI-quota chip), campaigns CRUD (create incl. one-click start_now)+edit+duplicate+performance page (incl. per-A/B-variant retention summary), AI campaign designer endpoint (downgrades auto-music when no Freesound key), asset actions (approve / reject→learning / publish-now / discard&re-render), streaming, task API+retry, credential tests (incl. Freesound), Gemini model-chain save + live model listing endpoint, OAuth, read-only /api/summary snapshot (header attention badge + dashboard auto-refresh, reuses the dashboard helpers); master-detail drill-down (scoped list views via ?channel=/?campaign= query params on campaigns/assets/tasks + rollup counts for the Channel→Campaign→Assets links, Performance as the campaign hub); dashboard triage inbox + activity feed context (attention_failed/attention_review); template globals: settings + voice_choices | HTTP | HTML/JSON | fastapi, auth, models, task_queue, ai_engine, tts, usage | DONE | 2026-07-22 |
| `templates/` | web | Jinja2 dashboard (7 pages incl. Calendar) + login.html, built on `macros.html` (pill/card/stat/progress/bar/banner/empty/field components) and the base shell (intent-grouped nav Monitor/Content/Setup, mobile drawer). Persona-ordered pages with full empty/loading/degraded states; Asset Pool video-first review cards + flash banners; guided campaign form (AI Propose + script Preview, per-language voice dropdown, progressive-disclosure advanced sections, sticky save bar, JS validation with tab-jump); Performance A/B retention bars + episode mini-bars; Credentials Gemini model-chain picker (live RPM/TPM/RPD list); accessible confirm dialog (data-confirm, optional type-to-confirm for channel delete); cross-page live attention badge + mobile bottom tab bar; Asset Pool filter chips + reject-reason quick-pick + keyboard review (J/K/A/R); client-side search on Campaigns & Task Logs; Performance retention sparkline; "Create & Start" one-click; master-detail navigation (breadcrumbs, drill-down links Channel→Campaign→Assets/Tasks, scoped list views, Performance-as-campaign-hub tab row); "trust-instrument" dashboard (triage inbox + inline retry, all-clear state, activity feed w/ since-last-visit); review-in-context criteria on Asset Pool; visible learning loop; live campaign identity card | context | HTML | jinja2 | DONE | 2026-07-22 |
| `static/` | web | `app.css` design system (dark-first + optional light theme via [data-theme]; tokens colour/type/spacing/radii/elevation; one semantic status-colour set reused by pills/banners/charts; 12-col grid; responsive off-canvas nav + mobile bottom tab bar; skeletons; hand-rolled CSS/SVG data-viz bars + sparkline; modal/toast/badge/chip components) + `ui.js` shared helpers (busy-button, drawer + theme toggle, /api/summary poller → attention badge + dashboard auto-refresh + "Live" pulse + reconnect toast, confirm dialog w/ type-to-confirm, toasts, relative-time) + polling `app.js` (401→/login, mobile card-stacking, client-side filter) — no runtime CDN | — | CSS/JS | — | DONE | 2026-07-22 |
